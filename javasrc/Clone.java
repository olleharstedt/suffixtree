...
class Clone extends KeyValueStoreBase {
	 ...
	 * element).
	 */
	private List<SimpleRegion> gaps;

	/** Delta size in units */
	protected int deltaInUnits;

	/** Creates a clone with a delta in units of 0 */
	public Clone(long id, TextRegionLocation location,
			int startUnitIndexInElement, int lengthInUnits, String fingerprint) {
		this(id, location, startUnitIndexInElement, lengthInUnits,
				fingerprint, 0);
	}
	...
	@Override
	public int hashCode() {
		return getStartUnitIndexInElement() * 17 + getLengthInUnits();
	}
	 ...
	 */
	public UnmodifiableList<SimpleRegion> getGapPositions() {
		if (gaps == null) {
	...
	/** Adds a gap position. */
	public void addGap(SimpleRegion gapRegion) {
		if (gaps == null) {
			gaps = new ArrayList<SimpleRegion>();
		}
		gaps.add(gapRegion);
	}
