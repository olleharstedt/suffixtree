...
class SuffixTree {
	...
	/** @var string[] The word we are working on. */
	protected $word;
	...
	public function __construct(array $word) {
		$this->word = $word;
		$size = count($this->word);
		$this->INFTY = $size;

		$expectedNodes = 2 * $size;
		$this->nodeWordBegin = [$expectedNodes];
		$this->nodeWordEnd = [$expectedNodes];
		$this->suffixLink = [$expectedNodes];
	 ...
	 */
	private function testAndSplit(int $refWordEnd, $nextCharacter) {
		...
		$this->nodeWordEnd[$this->explicitNode] = $this->nodeWordBegin[$next] + $refWordEnd
				- $this->refWordBegin;
	...
	protected function ensureChildLists() {
		if ($this->nodeChildFirst == null || $this->nodeChildFirst.length < $this->numNodes) {
			$this->nodeChildFirst = [$this->numNodes];
			$this->nodeChildNext = [$this->numNodes];
			$this->nodeChildNode = [$this->numNodes];
			$this->nextNode->extractChildLists($this->nodeChildFirst, $this->nodeChildNext,
					$this->nodeChildNode);
		}
	}

	/** Prints some internal numbers to std error. */
	protected function _dumpDebugInfos() {
		echo ("Number of nodes created: " + $this->numNodes);
		echo ("Hash table infos: ");
	 ...
	 */
	protected function _dumpAsDOT(string $separationChar, boolean $includeSuffixLinks) {
        /*
	...
	public function containsWord(array $find) {
		...
		for ($i = 0; $i < $findSize;) {
			...
			for ($j = $this->nodeWordBegin[$next]; $j < $this->nodeWordEnd[$next]
					&& $i < $findSize; ++$i, ++$j) {
				if (!$this->word[$j]->equals(find.get($i))) {
					return false;
				}
			}
			$node = $next;
